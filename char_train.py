...
def main(cfg: DictConfig):
    ...
    if getattr(cfg.dataset, "mask_zero_padding", False):
        accelerator.print(f"Masking zero-padding token (id={cfg.dataset.zero_padding_idx}) from training loss")
    accelerator.print()
    ...
    for it in bar:
        ...
        if getattr(cfg.dataset, "mask_zero_padding", False):
            ...
            targets[targets == cfg.dataset.zero_padding_idx] = -100
        ...
        if (it + 1) % grad_accum == 0:
            ...
            if accelerator.is_main_process:
                ...
                num_bytes = bytes_per_token[targets[valid_mask]].sum().item()
            ...
            _valid = targets[targets != -100]
            _num_bytes = bytes_per_token[_valid].sum().item()
            _ce = stats.get("reinforce/ce_loss", stats.get("hnet/ce_loss", loss.item()))
            _bpb = _ce * _valid.numel() / _num_bytes / math.log(2) if _num_bytes > 0 else 0.0
